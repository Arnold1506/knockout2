...
export const Dashboard = () => {
    ...
    let intervalFunction = async() =>{
        if (user && !handle) {
            axios.post('http://localhost:4000/api/findHandle', { username: user.email })
                .then(res => {
        ...
        axios.get(userInfoUrl)
            .then(res => {
                if (res.data.result[0].firstName === firstName) {
                    axios.post('http://localhost:4000/api/updateUserHandle', { handle: handleRef.current.value, username: user.email })
                        .then(res => {
    ...
    let handleUpdate = (e) => {
        if (window.confirm("Are you sure you want to update Handle ?")) {
            axios.post('http://localhost:4000/api/deleteHandle', { username: user.email })
                .then(res => {
    ...
    let roomValidate = () => {
        axios.post('http://localhost:4000/api/checkRoomCode', { roomCode: roomCodeRef.current.value })
            .then(res => {
